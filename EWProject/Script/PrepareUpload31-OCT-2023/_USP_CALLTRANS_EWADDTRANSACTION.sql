ALTER PROCEDURE "EW_PRD_TEST_31"._USP_CALLTRANS_EWADDTRANSACTION(
...
BEGIN
	...
	ELSE IF :DTYPE = 'UPDATENUMATCARDSQ' THEN
		execute immediate 'UPDATE 
								EW_PRD_TEST_31."OQUT" SET "NumAtCard"=(SELECT SUBSTRING_REGEXPR(''[^/]+'' IN '''||:par1||''' FROM 1 OCCURRENCE 1)||''/''||'''||:par2||''' from dummy) 
							WHERE "DocEntry"='''||:par2||''';';
	ELSE IF :DTYPE='AddReimbursement' THEN
		INSERT INTO EW_PRD_TEST_31."@TBREIMBURSEMENT"("Code","U_CardCode","U_Amount","U_Remark","U_DocStatus","U_CreateDate") 
			VALUES((EW_PRD_TEST_31."@TBREIMBURSEMENT_S".nextval),:par1,:par2,:par3,'P',CURRENT_DATE);
		SELECT CAST(MAX("Code") AS VARCHAR(30)) AS "DocEntry" FROM EW_PRD_TEST_31."@TBREIMBURSEMENT";
	ELSE IF :DTYPE='UpdateConfirmBookingStatus' THEN
		UPDATE EW_PRD_TEST_31."@TRUCKINFORMATION" SET "U_LineStatus"='Y' WHERE "DocEntry"=:par1 AND "LineId"=:par2;
		--SELECT CAST(:par1 AS VARCHAR(30)) AS "DocEntry" FROM Dummy;
		--SELECT * FROM EW_PRD_TEST_31."@TRUCKINFORMATION"
	ELSE IF :DTYPE='UpdateReimbursement' THEN
		UPDATE EW_PRD_TEST_31."@TBREIMBURSEMENT" SET "U_CardCode"=:par1,"U_Amount"=:par2,"U_Remark"=:par3 WHERE "Code"=:par4;
		SELECT CAST(:par4 AS VARCHAR(30)) AS "DocEntry" FROM Dummy;		
	ELSE IF :DTYPE='DeleteReimbursement' THEN
		DELETE FROM EW_PRD_TEST_31."@TBREIMBURSEMENT" WHERE "Code"=:par1;
		SELECT CAST(:par4 AS VARCHAR(30)) AS "DocEntry" FROM Dummy;
	ELSE IF :DTYPE='UpdateStatusReimbursement' THEN
		UPDATE EW_PRD_TEST_31."@TBREIMBURSEMENT" SET "U_DocStatus"=:par1,"U_Reason"=:par2,"U_UserApprove"=:par3 WHERE "Code"=:par4;
		SELECT CAST(:par3 AS VARCHAR(30)) AS "DocEntry" FROM Dummy;	
	ELSE IF :DTYPE = 'UPDATENUMATCARDSO' THEN
		UPDATE EW_PRD_TEST_31."ORDR" SET "NumAtCard"='' WHERE "DocEntry"=:par2;
		UPDATE EW_PRD_TEST_31."@JOBSHEETRUCKING" SET "U_SALESORDERDOCNUM"=NULL,"U_CONFIRMBOOKINGID"=NULL WHERE "DocEntry"=:par1;
	ELSE IF :DTYPE = 'UpdateLineStatusInactivePRCOS' THEN
		UPDATE EW_PRD_TEST_31."@TRUCKINFORMATION" SET "U_LineStatus"=''
		WHERE "DocEntry"= (SELECT DISTINCT
							 (SELECT
								T0."DocEntry"
							  FROM EW_PRD_TEST_31."@TRUCKINFORMATION" AS T0
							  LEFT JOIN EW_PRD_TEST_31."@CONFIRMBOOKINGSHEET" AS T1 ON T1."DocEntry"=T0."DocEntry"
						...
						FROM EW_PRD_TEST_31."@ADVANCEPAYMENTROW" AS A
						LEFT JOIN EW_PRD_TEST_31."@ADVANCEPAYMENT" AS C ON C."DocEntry"=A."DocEntry"
						WHERE A."DocEntry"=:par1 AND A."U_ItemCode"='01-T-1-0001')
			  AND "LineId"=:par2;
	ELSE IF :DTYPE = 'UpdateLineStatusActivePRCOS' THEN
		UPDATE EW_PRD_TEST_31."@TRUCKINFORMATION" SET "U_LineStatus"='Y'
		WHERE "DocEntry"= (SELECT DISTINCT
							 (SELECT
								T0."DocEntry"
							  FROM EW_PRD_TEST_31."@TRUCKINFORMATION" AS T0
