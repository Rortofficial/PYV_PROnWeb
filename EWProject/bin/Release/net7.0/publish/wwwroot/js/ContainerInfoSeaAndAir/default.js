function onChangeContainerType(id, action) {
    ...
    //alert(id);
    if (a === "FreeKey") {
        $("#plTruckTypeFreeKey-" + id).css("display", "");
        $("#plTruckType-" + id).css("display", "none");
        $('#cboTruckNo-'+id).val("").trigger("change");
        $('#cboProvinceTruck-'+id).val("").trigger("change");
    } else {
        $("#plTruckTypeFreeKey-" + id).css("display", "none");
        $("#plTruckType-" + id).css("display", "");
        $("#cboProvinceTruckFreeKey-" + id).val("");
        $("#cboTruckNoFreeKey-" + id).val("");
        $("#txtTrcukTypeFreeKey-" + id).val("");
        $("#txtBrandFreeKey-" + id).val("");
        $("#txtColorFreeKey-" + id).val("");
        $("#txtTralerProvinceFreeKey-" + id).val("");
        $("#txtTralerNoFreeKey-" + id).val("");
        $("#txtTrailerTypeFreeKey-" + id).val("");
    }
    //alert("Hello");
}
...
function onEditConfirmBookingSheet(id,data) {
    ...
    for (var i = 0; i < obj.length; i++) {
        ...
        $("#cboContainerNoTEST-" + id).val(obj[i].containerNo);
        //$("#cboTruckNo-" + id).val(obj[i].truckPlateNo).trigger("change");
        //$("#cboTruckNo-" + id).val(obj[i].truckPlateNo).trigger("change");
        if (obj[i].type == "FreeKey") {
            $("#cboProvinceTruckFreeKey-" + id).val(obj[i].truckProvince);
            $("#cboTruckNoFreeKey-" + id).val(obj[i].truckPlateNo);
            $("#txtTrcukTypeFreeKey-" + id).val(obj[i].truckType);
            $("#txtBrandFreeKey-" + id).val(obj[i].brand);
            $("#txtColorFreeKey-" + id).val(obj[i].color);
            $("#txtTralerProvinceFreeKey-" + id).val(obj[i].trailerProvince);
            $("#txtTralerNoFreeKey-" + id).val(obj[i].trailerPlate);
            $("#txtTrailerTypeFreeKey-" + id).val(obj[i].trailerType);
        } else {
            $("#cboTruckNo-" + id).val(obj[i].truckPlateNo).trigger("change");
        }
